...
def run_interactive_test():
    ...
    print(f"\n{Fore.CYAN}🎯 Running interactive test...{Style.RESET_ALL}")
    
    try:
        from ai_assistant import AIAssistant
        
        print("Starting AI Assistant for interactive test...")
        print("Type 'test' to test input processing, or 'exit' to quit")
        
        assistant = AIAssistant()
        
        while True:
            try:
                user_input = input(f"{Fore.YELLOW}Test ({assistant.current_mode.value}): {Style.RESET_ALL}").strip()
                
                if user_input.lower() == 'exit':
                    break
                elif user_input.lower() == 'test':
                    assistant.process_input("Hello, this is a test message")
                else:
...
def main():
    ...
    if passed == total:
        ...
        response = input(f"\n{Fore.YELLOW}Would you like to run an interactive test? (y/n): {Style.RESET_ALL}")
